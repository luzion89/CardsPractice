...
- AI prompt 现在会提交结构化轮次信息，显式标注每轮领出方以及 self / partner / opponent 关系，减少误判敌我
- AI 获取出牌信息失败时会在后台自动重试 3 次；重试仍失败则前端报错，不会直接当作过牌
...
- AI 每次请求都会携带结构化轮次数据，标明领出方和敌我关系，并在 prompt 中强调“尽量保留三张/连对/顺子等结构，不要为了走单牌随意拆牌”。
- AI 出牌信息获取失败时会后台自动重试 3 次；只有重试仍失败时才向前端报错。
